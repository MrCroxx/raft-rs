...
#[tokio::test]
async fn test_sending_snapshot_set_pending_snapshot() {
    let l = default_logger();
    let mut sm = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l).await;
    sm.restore(testing_snap()).await;
    sm.persist().await;
...
#[tokio::test]
async fn test_pending_snapshot_pause_replication() {
    let l = default_logger();
    let mut sm = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l).await;
    sm.restore(testing_snap()).await;
    sm.persist().await;
...
#[tokio::test]
async fn test_snapshot_failure() {
    let l = default_logger();
    let mut sm = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l).await;
    sm.restore(testing_snap()).await;
    sm.persist().await;
...
#[tokio::test]
async fn test_snapshot_succeed() {
    let l = default_logger();
    let mut sm = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l).await;
    sm.restore(testing_snap()).await;
    sm.persist().await;
...
async fn do_test_snapshot_with_min_term(pre_vote: bool, l: &slog::Logger) {
    let s = new_storage();
    s.wl()
        .apply_snapshot(new_snapshot(1, 1, vec![1, 2]))
        .unwrap();
    let n1 = new_test_raft_with_prevote(1, vec![1, 2], 10, 1, s, pre_vote, l).await;
    let n2 = new_test_raft_with_prevote(2, vec![], 10, 1, new_storage(), pre_vote, l).await;
    let mut nt = Network::new(vec![Some(n1), Some(n2)], l).await;
    nt.send(vec![new_message(1, 1, MessageType::MsgHup, 0)])
        .await;
    // 1 will be elected as leader, and then send a snapshot and an empty entry to 2.
    assert_eq!(nt.peers[&2].raft_log.first_index().await, 2);
    assert_eq!(nt.peers[&2].raft_log.last_index().await, 2);
}

// Initialized storage should be at term 1 instead of 0. Otherwise the case will fail.
#[tokio::test]
async fn test_snapshot_with_min_term() {
    let l = default_logger();
    do_test_snapshot_with_min_term(true, &l).await;
    do_test_snapshot_with_min_term(false, &l).await;
}

#[tokio::test]
async fn test_request_snapshot() {
    let l = default_logger();
    let mut sm = new_test_raft(1, vec![1, 2], 10, 1, new_storage(), &l).await;
    sm.restore(testing_snap()).await;
    sm.persist().await;
    ...
    sm.step(out_of_order).await.unwrap();
    assert_eq!(sm.prs().get(2).unwrap().state, ProgressState::Replicate);
